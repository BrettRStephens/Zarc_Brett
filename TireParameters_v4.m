...
cd '/Users/BrettStephens/Documents/Zarc_Brett/measurements_19Dec2017';
...
for j = 1:1%length(files)
    ...
    idx_straight = find(dxB_f > 0.014,1);
    t_straight = t(idx_straight); %starting going straight
    t_s = t_straight + 2; %start steering 2 seconds after start
    idx_s = find(t >= t_s,1);
    t_e = t_s + 10; %10 seconds later, when steering sweep is done
    idx_e = find(t >= t_e,1);
